...
tree_model_rating = DecisionTreeRegressor(random_state=42)
tree_model_share = DecisionTreeRegressor(random_state=42)

# Train the models
tree_model_rating.fit(X_train_rating_scaled, y_rating_train)
tree_model_share.fit(X_train_share_scaled, y_share_train)

# Predict on the full test set (to evaluate the model)
rating_pred = tree_model_rating.predict(X_test_rating_scaled)
share_pred = tree_model_share.predict(X_test_share_scaled)

# Calculate mse on the full test set
mse = mean_squared_error(y_share_test, share_pred)
rmse_share = np.sqrt(mse)
r2_score_share = tree_model_share.score(X_test_share_scaled, y_share_test)

mse = mean_squared_error(y_rating_test, rating_pred)
rmse_rating = np.sqrt(mse)
r2_score_rating = tree_model_rating.score(X_test_rating_scaled, y_rating_test)
...
X = df[['DURASI', 'KATEGORI_NUMBER', 'PRESENTER_NUMBER']]
y_share = df['SHARE']  # Target 1
y_rating = df['RATING']    # Target 2
...
if time_range_input:
    ...
    for presenter in unique_presenter:
        for kategori in unique_kategori:
            ...
            predicted_rating = tree_model_rating.predict(input_data)
            predicted_share = tree_model_share.predict(input_data)
...
if time_range_input:
    ...
    st.header("Prediksi dengan feature 'DURASI', 'KATEGORI', 'PRESENTER'")
    for presenter in unique_presenter:
        for kategori in unique_kategori:
            kategori_encoded = label_encoder_kategori.transform([kategori])
            presenter_encoded = label_encoder_presenter.transform([presenter])
